	...
	'function' == typeof define && define.amd
		...
		: 'object' == typeof module && module.exports
			? (module.exports = function(n, i) {
					return (
						void 0 === i && (i = 'undefined' != typeof window ? require('jquery') : require('jquery')(n)),
						e(i),
						i
					);
				})
	...
	(n.prototype = {
		...
		initEvents: function() {
			self.document.on('click.okayNav', function(n) {
				var i = e(n.target);
				self.nav_open === !0 && 0 == i.closest('.okayNav').length && self.closeInvisibleNav(),
					i.hasClass(self.options.toggle_icon_class) && (n.preventDefault(), self.toggleInvisibleNav());
			});
			...
			self.document
				.on('touchstart.okayNav', function(n) {
					if ((self.nav_invisible.removeClass('transition-enabled'), 1 == n.originalEvent.touches.length)) {
						...
						((i.pageX < 25 && 0 == self.options.align_right) ||
							(i.pageX > e(self.options.parent).outerWidth(!0) - 25 && 1 == self.options.align_right) ||
							self.nav_open === !0) &&
							((self.sTouch.x = self.cTouch.x = i.pageX),
							(self.sTouch.y = self.cTouch.y = i.pageY),
							(self.sTime = Date.now()));
					}
				})
				...
				.on('touchend.okayNav', function(e) {
					(self.sTouch = { x: 0, y: 0 }),
						(self.cTouch = { x: 0, y: 0 }),
						(self.sTime = 0),
						self.percent_open > 100 - self.options.threshold
							? ((self.nav_position = 0), self.closeInvisibleNav())
							: 1 == self.nav_moving &&
								((self.nav_position = self.nav_invisible.width()), self.openInvisibleNav()),
						(self.nav_moving = !1),
		...
		_triggerMove: function(e, n) {
			...
			if (((self.sTouch.x = e), (self.sTouch.y = n), 20 > f)) {
				...
				var _ = r - (self.nav_position + v * o + d),
					p = _ / r * 100;
				(self.nav_position += v * o + d),
					(self.percent_open = p),
					self.nav_invisible.css('transform', 'translateX(' + (self.options.align_right ? 1 : -1) * p + '%)');
			}
		},
		getParent: function() {
		...
		recalcNav: function() {
			...
			return n > a
				...
				: (l > 0 && t >= s && o >= n && self._collapseNavItem(),
					...
					void (0 == self.getHiddenItemCount()
		...
		_collapseNavItem: function() {
			...
			(self.last_visible_child_width = n.outerWidth(!0)),
				self.document.trigger('okayNav:collapseItem', n),
				n.detach().prependTo(self.nav_invisible),
				self.options.itemHidden.call(),
				self.recalcNav();
		},
		_expandNavItem: function() {
			var n = e('li:first-child', self.nav_invisible);
			self.document.trigger('okayNav:expandItem', n),
				n.detach().appendTo(self.nav_visible),
			...
			if ('string' == typeof s && '_' !== s[0] && 'init' !== s) {
				...
				return (
					this.each(function() {
						...
						o instanceof n &&
							'function' == typeof o[s] &&
							(t = o[s].apply(o, Array.prototype.slice.call(l, 1))),
							'destroy' === s && e.data(this, 'plugin_' + i, null);
					}),
					void 0 !== t ? t : this
				);
			}
